...
import { useCatalogTourBooking } from "../model/hooks/use-catalog-tour-booking";

import { BookingStepperCarousel } from "./booking-stepper-carousel";
import { CatalogTourBookingSidebar } from "./catalog-tour-booking-sidebar";
...
const BOOKING_FORM_ID = "catalog-tour-booking-form";

interface ICatalogTourBookingProps {
	tourId: string;
	bookingId?: string;
}

const CatalogTourBookingBase: FC<ICatalogTourBookingProps> = ({
	tourId,
	bookingId
}) => {
	...
	const {
		...
		isOptionsLoading
	} = useCatalogTourBooking({ tourId, bookingId });
			...
			<div className="w-full py-8">
				{currentStep < LAST_STEP && (
					<Button
						...
						onClick={() =>
							router.push(
								buildRoute(ENUM_PATH.CATALOG.TOUR, { tourId })
							)
						}
					>
...
export const CatalogTourBooking = withErrorBoundary(CatalogTourBookingBase);
