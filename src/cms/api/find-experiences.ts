...
): Promise<{ themeId?: number; countryId?: number; cityId?: number }> {
	const payload = await getPayload({ config });
	const result: { themeId?: number; countryId?: number; cityId?: number } =
		{};
...
function buildExperienceWhere(
	filters: TExperienceListFilters,
	ids: { themeId?: number; countryId?: number; cityId?: number }
): Where {
