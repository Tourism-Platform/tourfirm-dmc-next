...
const mapFilesToState = (
	files: (TFileMetadata | File)[],
	existing?: TFileWithPreview[]
): TFileWithPreview[] =>
	files.map((file, index) => {
		...
		return {
			file,
			id: file instanceof File ? `${file.name}-${index}` : file.id,
			preview: file instanceof File ? undefined : file.url
		};
	});

const getInitialFilesKey = (files: (TFileMetadata | File)[]) =>
	files
		.map((file) =>
			file instanceof File ? `${file.name}:${file.size}` : file.id
		)
		.join("|");
...
export const useFileUpload = (
	...
	const [state, setState] = useState<TFileUploadState>(() => ({
		files: mapFilesToState(initialFiles),
		isDragging: false,
		errors: []
	}));

	const inputRef = useRef<HTMLInputElement>(null);
	const initialFilesKey = getInitialFilesKey(initialFiles);
	const [prevInitialFilesKey, setPrevInitialFilesKey] =
		useState(initialFilesKey);

	if (initialFilesKey !== prevInitialFilesKey && initialFiles.length > 0) {
		setPrevInitialFilesKey(initialFilesKey);
		setState((prev) => {
			...
			return {
				...prev,
				files: mapFilesToState(initialFiles, prev.files)
			};
		});
	}
