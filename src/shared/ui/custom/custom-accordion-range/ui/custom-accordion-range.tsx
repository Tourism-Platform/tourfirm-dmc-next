...
type TCustomAccordionRangeProps = {
	id: string;
	title: string;
	icon?: LucideIcon;
	min: number;
	...
	isLoading?: boolean;
	onChange: (value: TPriceRange) => void;
	className?: string;
};

export const CustomAccordionRange: FC<TCustomAccordionRangeProps> = memo(
	({
		id,
		title,
		icon: Icon,
		...
		const [localValue, setLocalValue] = useState<TPriceRange>({
			from: from ?? min,
			to: to ?? max
		});
		...
		const handleLiveSliderChange = useCallback((newValue: TPriceRange) => {
			setLocalValue(newValue);
		}, []);

		const handleSliderCommit = useCallback(
			(newValue: TPriceRange) => {
				setLocalValue(newValue);
				onChange(newValue);
			},
			[onChange]
		...
		const skeletonCount = Math.max(1, Math.round((max - min) / step));
