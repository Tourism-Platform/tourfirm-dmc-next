/**
 * Apply Bukhara-style UX to all content/cities/*.yml:
 * - short hero + form/catalog CTAs
 * - "About the city" quote + dashTitle block after hero
 * - fill empty mapPanel.description
 * - end CTA with image + form + catalog
 *
 * Idempotent. Usage: node scripts/patch-all-cities.mjs
 */
import fs from "node:fs";
import path from "node:path";
import YAML from "yaml";

const DIR = path.join("content", "cities");
...
function aboutBlockYaml(doc) {
	...
	const productBody = asideDesc
		...
		: L(
				`Keep one clear city core, then add extensions that fit the season — ${subtitle.en}.`,
				`Одно понятное городское ядро, затем расширения под сезон — ${subtitle.ru}.`,
				`Bitta aniq shahar yadrosi, keyin mavsumga mos kengaytmalar — ${subtitle.uz}.`
			);
...
function ctaBlockYaml(doc) {
    ...
    "description":
      "en": "We help agencies set a realistic pace and choose the right next move — without rushing the places that matter."
      "ru": "Помогаем агентствам выстроить реалистичный темп и выбрать следующий шаг — без спешки там, где важны места."
      "uz": "Agentliklarga real sur’at va keyingi qadamni tanlashda yordam beramiz — muhim joylarni shoshiltirmasdan."
    "actions":
      - "type": "form"
        "variant": "default"
        "title":
...
function ensureAbout(source, doc) {
	if (source.includes('"ru": "О городе"')) {
...
function fillMapPanelDescription(source, doc) {
	...
	const desc = L(fallback.en || "", fallback.ru || "", fallback.uz || "");
