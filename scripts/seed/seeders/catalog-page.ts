...
export async function seedCatalogPage(payload: Payload): Promise<void> {
	...
	for (const locale of LOCALES) {
		...
		if (!localeData) {
			console.log(`  ~ skip catalog locale ${locale}`);
			continue;
		}

		await updateGlobalLocale(payload, "catalog", locale, {
			seo: localeData.seo ?? {},
